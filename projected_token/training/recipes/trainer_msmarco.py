#!/usr/bin/env python3
"""Trainer for OSCAR Projector - train on MS MARCO v1, validate on same dataset.

This trainer:
1. Trains on MS MARCO v1 dataset
2. Validates on MS MARCO v1 validation split
...
class TrainDataset(Dataset):
    """Training dataset for MS MARCO v1."""
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "negatives": negatives,
            "domain": item.get("domain", "msmarco"),
        }
...
class MSMarcoV1Trainer:
    """Trainer for MS MARCO v1 with validation on same dataset."""
    ...
    def __init__(
        ...
        device: str = "cuda:0",
        output_dir: str = "./checkpoints/projector_msmarco_v1",
        log_dir: str = "./logs/projector_msmarco_v1",
    ...
    def validate(self, step: int) -> Dict[str, float]:
        ...
        print(f"Val - Loss: {avg_loss:.4f}, MRR: {avg_mrr:.4f}")
        print(f"Val - R@1: {avg_recall_at_1:.4f}, R@5: {avg_recall_at_5:.4f}, R@10: {avg_recall_at_10:.4f}")
    ...
    def train(self, num_epochs: int, val_every_n_steps: int = 500):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for batch in pbar:
                ...
                if step % val_every_n_steps == 0:
                    ...
                    if is_best:
                        best_val_mrr = val_metrics["mrr"]
                        self.epochs_without_improvement = 0
                        print(f"New best model! Val MRR: {best_val_mrr:.4f}")
                    else:
                    ...
                    if self.epochs_without_improvement >= self.early_stopping_patience:
                        print(f"\nEarly stopping triggered after {self.epochs_without_improvement} validations without improvement")
            ...
            self.writer.add_scalar("train/epoch_mrr", epoch_mrr, epoch)
...
def main():
    parser = argparse.ArgumentParser(description="Train on MS MARCO v1, validate on same dataset")
    ...
    parser.add_argument("--output-dir", type=str,
                        default="./checkpoints/projector_msmarco_v1")
    parser.add_argument("--log-dir", type=str,
                        default="./logs/projector_msmarco_v1")
    parser.add_argument("--early-stopping-patience", type=int, default=7,
                        help="Number of validations without improvement before early stopping")
    
    args = parser.parse_args()
    
    train_dataset = TrainDataset(args.train_dataset)
    val_dataset = ValDataset(args.val_dataset)
    
    trainer = MSMarcoV1Trainer(
