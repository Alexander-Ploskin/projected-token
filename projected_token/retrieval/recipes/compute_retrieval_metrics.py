...
def main():
    ...
    parser.add_argument("--encoder", required=True, choices=["oscar", "salesforce", "oscar_projector"])
    parser.add_argument("--model-name-or-path", required=True)
    parser.add_argument("--projector-path", default=None, help="Путь к чекпоинту проектора (для oscar_projector)")
    ...
    for i in tqdm(range(0, len(all_queries), args.batch_size), desc="Поиск"):
        ...
        with torch.inference_mode():
            if args.encoder in ("salesforce", "oscar_projector"):
                query_embeddings = encoder.encode(documents=batch_questions, questions=None)
            else:
