...
def main():
    ...
    parser.add_argument(
        "--encoder",
        required=True,
        choices=["oscar", "salesforce", "oscar_projector"],
        help="Тип энкодера"
    )
    parser.add_argument(
        "--model-name-or-path",
    ...
    parser.add_argument(
        "--projector-path",
        default=None,
        help="Путь к чекпоинту проектора (для oscar_projector)"
    )
    parser.add_argument(
        "--device",
        default="cuda:0",
    ...
    parser.add_argument(
        "--aggregation",
        default="mean",
        choices=["mean", "first", "last", "max", "mean_max", "flatten"],
        help="Стратегия агрегации mem-токенов: mean, first, last, max, mean_max, flatten"
    )
    parser.add_argument(
        "--pooler",
        default="mean",
        choices=["mean", "first", "last", "max", "mean_max", "flatten"],
        help="Pooler type for projector: mean, first, last, max, mean_max, flatten"
    )
    parser.add_argument(
        "--num-layers",
        type=int,
        default=2,
        help="Number of layers in projector"
    )
    parser.add_argument(
        "--dropout",
        type=float,
        default=0.1,
        help="Dropout in projector"
    )
    ...
    elif args.encoder == "oscar_projector":
        from projected_token.encoders import OscarProjectorEncoder
        encoder = OscarProjectorEncoder(
            oscar_model_name=args.model_name_or_path,
            projector_path=args.projector_path,
            device=args.device,
            embed_dim=768,
            pooler=args.pooler if args.pooler else args.aggregation,
