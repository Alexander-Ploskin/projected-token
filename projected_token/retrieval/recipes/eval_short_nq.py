#!/usr/bin/env python3
"""Evaluate OSCAR + Projector on short Natural Questions passages (<=70 tokens)."""
...
def load_nq_short(max_tokens: int = 70, max_samples: int = 500):
    """Load short passages from Natural Questions."""
    print(f"Loading Natural Questions (max {max_tokens} tokens, max {max_samples} samples)...")
    ...
    short_samples = []
    for i in tqdm(range(num_rows), desc="Filtering by token length"):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate on short Natural Questions passages")
    ...
    # Load short NQ passages
    dataset = load_nq_short(args.max_tokens, args.max_samples)
    ...
    for i, query in enumerate(tqdm(questions, desc="Evaluating")):
        ...
        # Compute similarity to top-10 negatives
    ...
    # Print results
    print("\n" + "=" * 60)
    print("RETRIEVAL METRICS ON SHORT NATURAL QUESTIONS (<=70 tokens)")
    ...
    # Save results
    output_path = "short_nq_retrieval_metrics.json"
