...
MASTER_LOG="$LOG_DIR/run_e6_flatten_candidates_${TS}.log"

exec > >(tee -a "$MASTER_LOG") 2>&1

echo "[E6][START] $(date -Is) container=$CONTAINER"

if ! docker ps --format '{{.Names}}' | awk -v c="$CONTAINER" '$0==c{found=1} END{exit(found?0:1)}'; then
  echo "[E6][ERROR] container $CONTAINER not running"
  exit 1
fi
...
if [[ -z "$HF_TOKEN_VALUE" ]]; then
  echo "[E6][WARN] HF token missing; gated models may fail"
fi

SESSION="stage_b_e6_flatten"
CFG="configs/training/e6_flatten_candidates.yaml"
...
echo "[E6][OK] launch requested"
echo "[E6][NEXT] tail -f artifacts/logs/${SESSION}.log"
