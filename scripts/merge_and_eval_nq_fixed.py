...
def load_oscar_split(compressor_device: str, decoder_device: str):
    ...
    # Load directly on decoder device to avoid accelerate hooks issues
    oscar_model = AutoModel.from_pretrained(
        "naver/oscar-qwen2-7B",
        torch_dtype=torch.bfloat16,
        trust_remote_code=True,
        device_map=decoder_device,
    ).eval()
    
    # Only move compressor if it's on a different device
    if compressor_device != decoder_device and hasattr(oscar_model, "compr"):
        oscar_model.compr = oscar_model.compr.to(compressor_device)
        from projected_token.oscar_runtime import _patch_compress_documents_to_compressor_device
        _patch_compress_documents_to_compressor_device(oscar_model)
...
def main():
    ...
    parser.add_argument(
        "--out-dir",
        type=str,
        default="artifacts/results/retrieval/beir_nq_e9_6000_fixed",
    )
    ...
    print("Loading shards for FAISS index...")
    all_embeddings = []
    all_ids = []
