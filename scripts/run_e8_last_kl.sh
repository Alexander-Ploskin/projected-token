...
MASTER_LOG="$LOG_DIR/run_e8_last_kl_${TS}.log"

exec > >(tee -a "$MASTER_LOG") 2>&1

echo "[E8][START] $(date -Is) container=$CONTAINER"

if ! docker ps --format '{{.Names}}' | awk -v c="$CONTAINER" '$0==c{found=1} END{exit(found?0:1)}'; then
  echo "[E8][ERROR] container $CONTAINER not running"
  exit 1
fi
...
if [[ -z "$HF_TOKEN_VALUE" ]]; then
  echo "[E8][WARN] HF token missing; gated models may fail"
fi

SESSION="stage_b_e8_last_kl"
CFG="configs/training/e8_last_kl.yaml"
...
echo "[E8][OK] launch requested"
echo "[E8][NEXT] tail -f artifacts/logs/${SESSION}.log"
