...
MASTER_LOG="$LOG_DIR/run_e5_token_global_${TS}.log"

exec > >(tee -a "$MASTER_LOG") 2>&1

echo "[E5][START] $(date -Is) container=$CONTAINER"

if ! docker ps --format '{{.Names}}' | awk -v c="$CONTAINER" '$0==c{found=1} END{exit(found?0:1)}'; then
  echo "[E5][ERROR] container $CONTAINER not running"
  exit 1
fi
...
if [[ -z "$HF_TOKEN_VALUE" ]]; then
  echo "[E5][WARN] HF token missing; gated models may fail"
fi

SESSION="stage_b_e5_token_global"
CFG="configs/training/e5_token_global.yaml"
...
echo "[E5][OK] launch requested"
echo "[E5][NEXT] tail -f artifacts/logs/${SESSION}.log"
